...
<div>
  <nav class="w-full bg-gradient-to-r from-blue-600 to-blue-500 text-white px-4 py-4 text-xl ">
    ...
    <button v-if="!isLogedIn" @click="$emit('open-login-modal')" class="bg-gray-300 text-green-700 font-semibold rounded inline-flex items-center p-2 float-right mx-2 -my-2">Log In</button>
    <div v-if="isLogedIn" class="dropdown inline-block relative float-right text-left -my-2">
      <button class="bg-gray-300 text-green-700 font-semibold rounded inline-flex items-center">
