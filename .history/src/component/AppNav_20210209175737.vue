...
<div>
  <nav class="w-full bg-gradient-to-r from-blue-600 to-blue-500 text-white px-4 py-4 text-xl ">
    ...
    <button v-if="!storeisLogedIn" @click="$emit('open-login-modal')" class="bg-green-300 text-white font-semibold rounded inline-flex items-center p-2 float-right mx-2 -my-2">Log In</button>
    <div v-if="isLogedIn" class="dropdown inline-block relative float-right text-left -my-2">
      <button class="bg-green-300 text-white font-semibold rounded inline-flex items-center">
        ...
        <div class="">
          <span v-if="authUser.displayName" class="px-2 float-left">{{ authUser.displayName }}</span><br>
          <small v-if="authUser.displayName" class="px-2 -my-2">{{ authUser.email }}</small>
        </div>
