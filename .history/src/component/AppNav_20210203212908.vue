<template>
  <nav class="w-full bg-gradient-to-r from-blue-600 to-blue-500 text-white px-4 my-4 text-xl ">
    <router-link to="/home" class="mx-2">Home</router-link>
    <router-link to="/page" class="mx-2">Page</router-link>
    <img v-if="isLogedIn" :src="authUser.photoURL"  width="50" height="50" class="lazy float-right"/>
    <span class="float-right mx-2">{{ authUser.displayName }}</span>
