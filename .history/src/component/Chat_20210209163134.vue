  ...
  <section class="flex w-full">
    <div class="m-auto">
      ...
      <div class="border rounded-lg">
        <div class="h-64 p-2">
          <div v-for="chat in state.chats" :key="chat.message" class="w-full text-right" :class="chat.userId === state.userId ? 'text-right' : '' ">{{chat.message}}</div>
        </div>
        <div class="h-8 p-2">
...
setup(){
  ...
  onMounted(async () => {
    ...
    chatRef.on("child_added",(snapshot)=>{
      ...
      state.chats.push({key: snapshot.key, ...snapshot})
